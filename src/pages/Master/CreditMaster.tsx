...
const CreditMaster: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCurrencies = async () => {
      try {
        const response = await api.get(`${BACKEND_API_KEY}/master/currencies`);
  ...
  const fetchCreditPrices = async () => {
    try {
      ...
      const response = await api.get(
        `${BACKEND_API_KEY}/master/credit-prices`,
        {
          params: {
            page: currentPage,
  ...
  const handleConfirmDelete = async () => {
    if (creditPriceIdToDelete !== null) {
      ...
      try {
        await api.delete(
          `${BACKEND_API_KEY}/master/credit-prices/${creditPriceIdToDelete}`
        );
        fetchCreditPrices();
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCreditPrice) {
        await api.put(
          `${BACKEND_API_KEY}/master/credit-prices/${editingCreditPrice.id}`,
          formData
        );
  ...
  const toggleStatus = async (id: number, currentStatus: string) => {
    ...
    try {
      await api.put(`${BACKEND_API_KEY}/master/credit-prices/${id}`, {
        status: newStatus,
      });
      fetchCreditPrices();
    } catch (err) {
